import { Copy, Check, MessageSquare, Send, MessageCircle, Mail, Shield, Zap } from 'lucide-react';
...
export const NegotiationScriptModal = ({ isOpen, onClose, script, clause, explanation, persona }: NegotiationScriptModalProps) => {
  ...
  const handleGenerate = async (toneToUse?: 'Friendly' | 'Assertive' | 'Tough') => {
    ...
    try {
      ...
      toast.success(t('negotiationScript.toasts.scriptUpdated', { tone: activeTone }));
    } catch (error) {
      console.error('Generation Error:', error);
      toast.error(t('negotiationScript.toasts.updateFailed'));
    } finally {
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    toast.success(t('negotiationScript.toasts.scriptCopied'));
  ...
  const handleWhatsAppShare = () => {
    ...
    toast.success(t('negotiationScript.toasts.whatsappShare'));
  };
  ...
  const handleSendEmail = () => {
    ...
    toast.promise(
      ...
      {
        loading: t('negotiationScript.toasts.emailSending'),
        success: t('negotiationScript.toasts.emailSuccess'),
        error: t('negotiationScript.toasts.emailFailed'),
      }
    );
  };

  const toneOptions = [
    { id: 'Friendly' as const, label: t('negotiationScript.tones.friendly'), Icon: MessageCircle },
    { id: 'Assertive' as const, label: t('negotiationScript.tones.assertive'), Icon: Shield },
    { id: 'Tough' as const, label: t('negotiationScript.tones.tough'), Icon: Zap },
  ];
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-display font-semibold text-text">{t('negotiationScript.title')}</h3>
              <p className="text-[9px] text-text-subtle font-medium">{t('negotiationScript.subtitle')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          <label className="text-[9px] font-bold text-text-subtle tracking-wider ml-1">{t('negotiationScript.targetClause')}</label>
        ...
        <div className="flex flex-col gap-2 mb-4">
          <label className="text-[9px] font-bold text-text-subtle tracking-wider ml-1">{t('negotiationScript.negotiationStyle')}</label>
        ...
        {/* Script content */}
        <div className="flex flex-col gap-1.5 mb-4">
          <label className="text-[9px] font-bold text-text-subtle tracking-wider ml-1">{t('negotiationScript.aiSuggestedScript')}</label>
            ...
            <div className={`w-full bg-background border border-border rounded-xl p-4 text-xs text-text font-medium leading-relaxed min-h-[120px] whitespace-pre-wrap flex items-center justify-center ${isGenerating ? 'opacity-50' : ''}`}>
              {isGenerating ? (
                <div className="flex flex-col items-center gap-2 text-primary">
                  ...
                  <p className="text-[10px] font-bold animate-pulse">{t('negotiationScript.geminiWriting')}</p>
                </div>
              ) : currentScript ? (
                ...
                <div className="flex flex-col items-center gap-3 text-center py-2">
                  ...
                  <div>
                    <p className="text-[10px] text-text-muted font-bold">{t('negotiationScript.oldAnalysis')}</p>
                    <p className="text-[9px] text-text-subtle">{t('negotiationScript.generateScriptNow')}</p>
                  </div>
                  <button onClick={() => handleGenerate()} className="btn-primary py-1.5 px-4 text-[9px] flex items-center gap-2">
                    <RefreshCw className="w-2.5 h-2.5" />
                    {t('common.buttons.generateNow')}
                  </button>
                </div>
              )}
            </div>
            {currentScript && !isGenerating && (
              <button onClick={handleCopy} title={t('common.buttons.copyScript')} className="absolute top-2 right-2 p-2 bg-surface border border-border rounded-lg shadow-sm hover:border-primary/50 hover:bg-white transition-all group/btn active:scale-90">
        ...
        {currentScript && !isGenerating && (
          <div className="flex flex-col gap-2 mb-4">
            <label className="text-[9px] font-bold text-text-subtle tracking-wider ml-1">{t('negotiationScript.smartShareHub')}</label>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={handleWhatsAppShare} className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-green-500/20 bg-green-500/5 hover:bg-green-500/10 text-green-600 dark:text-green-400 font-bold text-[10px] transition-all hover:scale-[1.02] active:scale-98 cursor-pointer">
                ...
                {t('negotiationScript.sendToWhatsApp')}
              </button>
              <button onClick={handleSendEmail} className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold text-[10px] transition-all hover:scale-[1.02] active:scale-98 cursor-pointer">
                ...
                {t('negotiationScript.sendPdfToEmail')}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-end gap-2 pt-2 border-t border-border">
          ...
          <button onClick={handleCopy} disabled={!currentScript || isGenerating} className="btn-primary px-6 py-2 text-[10px] flex items-center gap-2">
            ...
            <span>{copied ? t('negotiationScript.copied') : t('common.buttons.copyScript')}</span>
          </button>
        </div>
      </div>
    </MorphingModal>
