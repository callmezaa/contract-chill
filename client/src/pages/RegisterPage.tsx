...
export const RegisterPage = () => {
  ...
  const getStrengthColor = (index: number) => {
    if (strengthScore < index + 1) return isDark ? 'bg-white/10' : 'bg-slate-200';
        ...
        <div className="w-full lg:w-[58%] p-6 sm:p-10 xl:p-14 flex flex-col justify-center relative">
          ...
          <div className="relative mb-5">
             ...
             <div className="relative flex justify-center text-sm">
                <span className={`px-4 text-[12px] sm:text-[13px] font-medium text-text-muted ${isDark ? 'bg-surface' : 'bg-white'}`}>Or sign up with email</span>
             </div>
          </div>
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-5">
            ...
            <div className="relative group">
              ...
              <button 
                ...
                className="absolute right-0 top-1.5 text-text-muted hover:text-primary transition-colors focus:outline-none z-10"
              >
            ...
            {password.length > 0 && (
              <div className="flex flex-col gap-1.5 px-1 -mt-3.5">
            ...
            <div className="flex items-start gap-2 -mt-1">
               <input type="checkbox" id="terms" required className={`w-4 h-4 mt-0.5 rounded border-slate-300 text-primary focus:ring-primary ${isDark ? 'bg-surface border-white/20' : 'bg-white'}`} />
               <label htmlFor="terms" className="text-[12px] sm:text-[13px] text-text-muted cursor-pointer select-none leading-tight">
            ...
            >
              <AnimatePresence mode="wait">
                {loading ? (
                  ...
                  <motion.span key="idle">Sign Up</motion.span>
                )}
              </AnimatePresence>
            </motion.button>
          </form>
